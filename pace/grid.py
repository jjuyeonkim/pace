...
class ExternalNetcdfGridConfig(GridInitializer):
    ...
    The output from FRE-NCTools is in the form of supergrid
    data, and so the longitude and latitude values must be
    strided in order to only collect the D-grid values for
    input.
